...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className={`fixed inset-0 bg-black backdrop-blur-sm flex items-center justify-center z-50 p-3 sm:p-4 transition-all duration-300 ${
      isAnimating ? 'bg-opacity-60' : 'bg-opacity-0'
    }`}>
      <div className={`bg-white rounded-xl sm:rounded-2xl shadow-2xl max-w-lg w-full p-6 sm:p-8 relative transition-all duration-300 ease-out mobile-modal ${
        isAnimating 
        ...
        <button
          onClick={onClose}
          className="absolute top-4 sm:top-6 right-4 sm:right-6 text-gray-400 hover:text-gray-600 transition-colors p-2 rounded-full hover:bg-gray-100 tap-target"
        >
          <X className="w-6 h-6" />
        </button>
        ...
        <div className="text-center mb-6 sm:mb-8">
          ...
          <h2 className="mobile-text-2xl font-bold text-gray-900 mb-3">{t('auth.modal.title')}</h2>
          <p className="mobile-text-base text-gray-600 mb-4">{t('auth.modal.subtitle')}</p>
          ...
          <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-lg sm:rounded-xl p-3 sm:p-4 mb-4 sm:mb-6">
            <div className="flex items-center justify-center space-x-2 mb-2">
              ...
              <span className="text-green-800 font-semibold mobile-text-base">{t('auth.modal.freeTrialBanner')}</span>
            </div>
          ...
          {error && (
            <div className="mb-4 p-3 sm:p-4 bg-red-50 border border-red-200 rounded-lg sm:rounded-xl flex items-center space-x-2 text-red-700">
          ...
          {authProviders.map((provider) => {
            ...
            return (
              <button
                ...
                className={`group w-full bg-white border-2 border-gray-200 hover:border-gray-300 text-gray-800 font-semibold py-4 sm:py-5 px-4 sm:px-8 rounded-xl sm:rounded-2xl transition-all duration-300 hover:shadow-lg hover:scale-[1.02] flex items-center justify-between disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 transform relative overflow-hidden tap-target`}
              >
                {isLoading ? (
                  <>
                    <div className="flex items-center justify-center w-full">
                      <Loader2 className="w-6 h-6 animate-spin text-gray-600 mr-3" />
                      <span className="mobile-text-base text-gray-600">Connecting to {provider.displayName}...</span>
                    </div>
                  </>
                ) : (
                  <>
                    <div className="flex items-center space-x-3 sm:space-x-4">
                      <div className="w-10 sm:w-12 h-10 sm:h-12 bg-gray-50 rounded-lg sm:rounded-xl flex items-center justify-center group-hover:bg-gray-100 transition-colors">
                        <img 
                      ...
                      <div className="text-left">
                        <div className="mobile-text-base font-semibold text-gray-900">
                        ...
                        <div className="text-xs sm:text-sm text-gray-500 hidden sm:block">{t('auth.modal.startTrialInstantly')}</div>
                      </div>
                    </div>
                    <div className="w-7 sm:w-8 h-7 sm:h-8 rounded-full bg-gray-100 flex items-center justify-center group-hover:bg-gray-200 transition-colors">
        ...
        <div className="pt-4 sm:pt-6 border-t border-gray-200">
          ...
          <div className="mb-4 sm:mb-6">
            <label className="flex items-start space-x-3 cursor-pointer">
              <input
                ...
                className="mt-1 w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500 tap-target"
              />
              <span className="text-xs sm:text-sm text-gray-700">
                I agree to the{' '}
                <a 
                  href="/saas-agreement" 
                  className="text-blue-600 hover:underline font-medium tap-target" 
                  target="_blank" 
            ...
            {/* Download Options */}
            <div className="mt-3 ml-7 flex flex-col sm:flex-row items-start sm:items-center space-y-1 sm:space-y-0 sm:space-x-4 text-xs text-gray-600">
              <span>Download:</span>
              <div className="flex flex-col sm:flex-row space-y-1 sm:space-y-0 sm:space-x-4">
                <a
                  ...
                  className="flex items-center space-x-1 text-blue-600 hover:text-blue-800 hover:underline transition-colors tap-target"
                >
          ...
          <div className="mb-4 sm:mb-6">
            <p className="text-xs text-gray-500 text-center">
              ...
              <a 
                href="/privacy-policy" 
                className="text-blue-600 hover:underline tap-target" 
                target="_blank" 
                rel="noopener noreferrer"
              >
                Privacy Policy
