  ...
  return (
    <div className="min-h-screen bg-porcelain relative font-instrument">
      ...
      <section className="pt-28 pb-10 md:pt-32 md:pb-14">
        <div className="max-w-5xl mx-auto px-6">
          ...
          <div className="max-w-2xl">
            <div className="flex items-center gap-2.5 text-navy/50 mb-3">
      ...
      <section className="pb-16 md:pb-20">
        <div className="max-w-5xl mx-auto px-6">
          <p className="text-navy/55 text-base md:text-lg mb-8 max-w-2xl">
          ...
          <div className="space-y-4">
            {steps.map((step, index) => {
              ...
              return (
                <div
                  key={index}
                  className="bg-white rounded-2xl p-7 md:p-8 border border-navy/[0.06] hover:border-navy/15 hover:shadow-lg transition-all duration-300"
                >
                  <div className="flex items-start gap-5">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-navy/60 text-[15px] md:text-base leading-relaxed mb-4">
                        {step.body}
                      </p>
                      <div className="bg-navy/[0.04] rounded-xl px-4 py-3 border border-navy/[0.06]">
                        <p className="text-xs uppercase tracking-widest font-semibold text-navy/40 mb-1">
                          {t('gettingStartedPage.steps.exampleLabel')}
                        </p>
                        <p className="text-navy/75 text-sm leading-relaxed">{step.example}</p>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      <section className="pb-16 md:pb-20">
        <div className="max-w-5xl mx-auto px-6">
          <div className="max-w-2xl mb-8">
            ...
            <p className="text-navy/55 text-base md:text-lg leading-relaxed">
              {t('gettingStartedPage.examples.intro')}
            </p>
          </div>

          <div className="bg-white rounded-2xl border border-navy/[0.06] overflow-hidden">
            <div className="hidden md:grid grid-cols-[260px_1fr] gap-4 px-6 py-3 bg-navy/[0.03] border-b border-navy/[0.07]">
            ...
            <ul className="divide-y divide-navy/[0.05]">
              {examples.map((row, index) => (
                <li
                  key={index}
                  className="md:grid md:grid-cols-[260px_1fr] gap-4 px-6 py-4"
                >
                  <p className="font-general font-semibold text-navy text-sm mb-1.5 md:mb-0">
                    {row.goal}
                  </p>
                  <p className="text-navy/65 text-sm leading-relaxed italic">{row.say}</p>
                </li>
              ))}
            </ul>
          </div>
        ...
        <div className="max-w-5xl mx-auto px-6">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {tips.map((tip, index) => (
              <div
                key={index}
                className="bg-white rounded-2xl p-6 border border-navy/[0.06]"
              >
                <div className="w-10 h-10 bg-navy/[0.06] rounded-xl flex items-center justify-center mb-4">
                  <CheckCircle2 className="w-5 h-5 text-navy/60" />
                </div>
                <h3 className="font-general font-bold text-navy text-base mb-2">
                  {tip.title}
                </h3>
                <p className="text-navy/55 text-sm leading-relaxed">{tip.body}</p>
              </div>
            ))}
          </div>
        </div>
